        ...
        if activate:
            ...
            if self.reverse:
                self.pca9685.set_pwm(
                    self.port, 0,
                    round(((self.max - self.min) / 2 + self.min) * 2) -
    ...
    def set(self, degrees, seconds=0):
        ...
        if seconds <= 0:
            ...
            if toff > self.position:
                ...
                if self.reverse:
                    self.pca9685.set_pwm(
                        self.port, 0, toff_middle - self.position + self.adjust)
                else:
                    self.pca9685.set_pwm(self.port, 0,
                                         self.position + self.adjust)
        ...
        if activate:
            ...
            if self.reverse:
                ...
                self.pca9685.set_pwm(self.right_port, 0,
                                     self.position + self.right_adjust)
            else:
                self.pca9685.set_pwm(self.left_port, 0,
                                     self.position + self.left_adjust)
                self.pca9685.set_pwm(
                    self.right_port, 0,
...
class Arm:
    ...
    def __init__(self,
        ...
        self.shoulder = TowMotors(parameters[1], positions[1], self.pca9685,
                                  None)
    ...
    def get(self):
        return (self.base.get(), self.shoulder.get(), self.elbow.get(),
                self.wrist.get(), self.wrench.get())
